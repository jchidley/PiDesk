...
| 0.3 Typed session service | Complete | Typed command/response/event coverage, pre-launch audited Pi 0.85.x validation, explicit lifecycle tests, and no raw protocol traversal in UI code. |
...
Upstream history supports keeping this patch narrow. The unmerged browsing PR [#1762](https://github.com/earendil-works/pi/pull/1762) combined listing/navigation with a projected tree schema, label mutation, branch-summary abort, shared formatting, and session-location changes; it was closed while a broader RPC refactor was planned. Issue [#8645](https://github.com/earendil-works/pi/issues/8645) independently confirms that `navigate_tree` remains absent after read-only `get_tree` shipped. Neither tagged v0.84.4 nor tagged v0.85.0 contains `list_sessions` or `navigate_tree`; the 0.85.0 RPC declarations, implementation, client, and documentation were rechecked during release adoption.
