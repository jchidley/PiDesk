...
The current implementation is an RPC chat client and does not yet provide full graphical feature parity with Pi's interactive TUI. It supports acceptance-aware prompts, basic steering, abort queue recovery, model and thinking selection, extension dialogs, atomic new/project/backend session replacement, restored active-path messages, usage display, and typed rendering of thinking, tools, diffs, retries, compaction, errors, and safe Markdown. Session browsing and branching, full queue presentation, explicit follow-up controls, images, command discovery, complete RPC extension UI, operational controls, transcript navigation, and responsive accessibility remain planned work. The evidence and deliberate exclusions are recorded in [the Pi TUI capability gap review](tui-gap-review.md).
